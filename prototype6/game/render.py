...
def _objective(surf, rect, ch, lit):
    colors = {"1": C.GOOD, "2": C.PLAN, "3": C.BAD}
    col = colors.get(ch, C.INK)
    # Dim if not lit, but keep distinct
    if not lit:
        col = tuple(max(0, int(c * 0.5)) for c in col)
    pygame.draw.circle(surf, col, rect.center, max(2, rect.width // 2 - 2))
