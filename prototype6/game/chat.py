...
There are objectives scattered on the map, listed as landmarks with priorities.
Your task is to complete as many objectives as you can before the day ends, prioritizing the highest priority tasks first.
You must return to the base pad before your steps run out. If you do not have enough steps to complete all objectives and return to base, it is okay to skip some. Survival (returning to the base pad) is more important than completing every objective.
Do not explore the map, as it is already fully revealed. Just drive to the objectives to complete them, and then drive back to the base pad.
