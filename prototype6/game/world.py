...
LABELS = {
    "H": "base pad",
    "1": "Objective (Priority: High)",
    "2": "Objective (Priority: Medium)",
    "3": "Objective (Priority: Low)"
}
...
class World:
    ...
    def _arrive(self):
        ...
        ch = self.here.at(x, y)
        if ch in "123":
            # Clear it from the map by mutating the row string
            row = list(self.here.rows[y])
            row[x] = "."
            self.here.rows[y] = "".join(row)
            self.say(f"Objective '{label_for(self, ch)}' at ({x},{y}) completed.")
