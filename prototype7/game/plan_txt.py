...
def main(argv=None):
    ...
    # A local, not world state: nothing in the simulation turns the rover, so the
    # facing is an argument to the route rather than a property of the machine.
    heading = HEADINGS[args.heading]
    goal = tuple(args.to)
    ...
    actions, _ = nav.route_actions(cells, heading)
    out = nav.write_plan(args.out, [(cells, start, heading, None)], goal)
