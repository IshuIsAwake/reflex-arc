...
def _fresh():
    ...
    On the 50, always. A 13x13 window centred SCOUT_RANGE from the middle of the 30
    comes back clamped at the edge, so the size and range checks below would be
    measuring the arena rather than the flyer.
    """
    C.use("50")
    return World()
...
def test_the_view_admits_the_flyer_exists():
    ...
    # The prompt and the view may only promise what is wired up. In prototype 7 this
    # asserted the opposite -- the flyer was the second thing that lifted fog, and
    # claiming it was the only one was a lie. Here it is the only one.
    ok &= check("the view says the flyer is the only thing that lifts fog",
                "only thing that lifts fog" in sight.REVEAL_RULE)
    ok &= check("...and that driving does not", "driving" in sight.REVEAL_RULE)
    return ok
...
if __name__ == "__main__":
    ...
    C.use(C.DEFAULT_ARENA)   # `_fresh` pins the 50; leave the module as we found it
