...
def todo_list(w):
    """Her own list, in the order she wrote it, struck items and all.

    They stay rather than disappearing so an id keeps its meaning all sol -- a list that
    renumbers underneath her is one she cannot strike from, and it also puts what she has
    already done in front of her, which is the point of crossing a thing off rather than
    binning it.

    The empty state names both calls. `why` collapsed to nothing the moment history
    stopped carrying an example of one (`prototype2/results.md`), and an empty block is a
    block with no example in it, so the invitation has to be in the words.
    """
    n = w.notes
    if not n.todos:
        return ['YOUR LIST FOR TODAY -- empty. todo("...") puts something on it and '
                'strike(n) crosses it off. It is wiped when the sol ends.']
    head = f"{n.open} still open" if n.open else "all crossed off"
    return [f"YOUR LIST FOR TODAY -- {len(n.todos)} items, {head}."] + [
        f"  {i}. [{'x' if struck else ' '}] {text}"
        for i, (text, struck) in enumerate(n.todos, 1)]


def kept(w):
    """The doc that survives the night, quoted back whole.

    Shown every request rather than only in the morning: `remember` replaces what is
    there, so a rewrite whose effect she cannot see is a call she has no way to check --
    which is the vanishing call this prototype has been bitten by three times.
    """
    if not w.notes.memory:
        return ['WHAT YOU WROTE DOWN TO KEEP -- nothing yet. remember("...") is the only '
                'thing that survives tonight; this conversation and the list above do '
                'not. The map and the finished work carry themselves.']
    return ["WHAT YOU WROTE DOWN TO KEEP -- yours, and yours to rewrite the moment it "
            "stops being true.",
            f"  {w.notes.memory}"]
