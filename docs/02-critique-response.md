...
deterministic A\* beats RL, when `course.md` §3 already uses A\* for waypoint routing and RL only for
1–3 s local motion. The Mars section attacks sim-to-real transfer, which `course.md` §8 explicitly
...
corridor planning, and adding stochasticity to it is a downgrade. `course.md` §8 already preferred
Mars; this closes it.
