"""scout -- the only way anything here sees ground at all.
...
**In prototype 7 these competed with driving, which also bought map. Here they do not
compete with anything -- the flyer is the only way to see.** That makes the three knobs
load-bearing in a way they were not before: they are no longer shaping a choice between
two ways of mapping, they are the entire budget of what can be known in a sol. The
prototype-7 sweep that set them measured the wrong world; see `settings.py`.
