r"""Run me:  ..\.venv\Scripts\python.exe game\main.py [--map grid|rle] [--think] [--selftest]
...
def read_flags(argv):
    ...
    `--selftest` posts real key events into the real loop -- if the rover moves
    on screen under it, the game is fine and the keyboard focus was the issue.
    """
    ...
    p.add_argument("--no-fog", action="store_true",
                   help="start with the whole map visible -- for training runs, where "
                        "the objective is meant to be in sight from the first turn")
    p.add_argument("--selftest", action="store_true",
                   help="drive itself briefly and quit")
    p.add_argument("--executor", choices=("teleport", "policy", "plan"),
                   default=S.EXECUTOR,
                   help="what a goto does: step it, drive it with the policy, or "
                        "only plan it and write the route file (default: "
                        f"{S.EXECUTOR})")
...
def main():
    ...
    pygame.display.set_caption(
        f"Reflex Arc -- prototype 5  ·  {S.MAP_FORMAT} map"
        + ("  ·  thinking, sampler unpinned" if S.MODEL_THINK else ""))
    screen = pygame.display.set_mode(render.fit_to_screen())
    ...
    nav.clear_plan(w)   # this session's plan file starts empty, never yesterday's
    if args.no_fog:
        # Deliberate, and it changes what the run is evidence of: with the map open
        # there is no exploration left to do, only routing.
        w.here.reveal_all()
    ...
    frames = 0
    # --selftest posts these through the real event queue, so the rover visibly
    # drives with no keyboard involved.
    SELFTEST_KEYS = {10: pygame.K_w, 40: pygame.K_a}
    ...
    while True:
        ...
        if selftest:
            frames += 1
            if frames in SELFTEST_KEYS:
                key = SELFTEST_KEYS[frames]
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, key=key))
                pygame.event.post(pygame.event.Event(pygame.KEYUP, key=key))
            if frames >= 90:
                print(f"selftest: rover at {w.pos}, steps={w.steps}")
                leave(False)
