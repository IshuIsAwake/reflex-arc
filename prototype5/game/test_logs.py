r"""The run directory, and the promise that a crash never costs a run.

    ..\.venv\Scripts\python.exe game\test_logs.py
...
def test_the_game_log_carries_what_the_chat_log_cannot():
    ...
    with tempfile.TemporaryDirectory() as d:
        run, w = rig(d)
        nav.goto(w, 25, 5)          # blocked by rock, so there is something to record
        w.next_day()
