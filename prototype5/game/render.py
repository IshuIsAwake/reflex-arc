...
# Enough pane to read a reply in without the wrapping turning it into a column of
# single words, and enough tile to still see which cell the rover is on.
MIN_CHAT = 420
MIN_TILE = 8


def fit_to_screen(pad=(40, 90)):
    """Shrink the tile and the chat pane until the window fits the desktop.
    ...
    whatever is left for the pane. Never grows either -- the numbers in `config.py`
    and `settings.py` stay the ceiling, so a big monitor gets what was designed and a
    small one gets the same layout, smaller.

    `pad` leaves room for the title bar and the taskbar, which the desktop size
    reported here does not subtract. Mutates `C.TILE` and `S.CHAT_W`, so call it once
    after `pygame.init()` and before anything reads either.
    """
    try:
        sw, sh = pygame.display.get_desktop_sizes()[0]
    except (pygame.error, IndexError):
        return window_size()        # no display to ask; leave the shipped numbers
    sw, sh = sw - pad[0], sh - pad[1]
...
def draw_world(surf, w, f, reel=None):
    ...
    for y in range(max(0, lo_y), min(a.h, hi_y + 1)):
        for x in range(max(0, lo_x), min(a.w, hi_x + 1)):
            ...
            if (x, y) in w.buttons:
                _button(surf, rect, lit, (x, y) in w.pressed)
...
def _button(surf, rect, lit, pressed):
    """The RL button: a ring with a dot while waiting, filled dim once pressed."""
    col = C.BUTTON if lit else C.BUTTON_DIM
    r = max(3, C.TILE // 2 - 3)
    if pressed:
        pygame.draw.circle(surf, col, rect.center, r)
    else:
        pygame.draw.circle(surf, col, rect.center, r, 2)
        pygame.draw.circle(surf, col, rect.center, max(1, r - 3))
...
def draw_map(surf, w, f, cursor):
    ...
    for y in range(a.h):
        for x in range(a.w):
            ...
            if (x, y) in a.marks:
                _x_mark(surf, r)
            if (x, y) in w.buttons:
                pygame.draw.circle(surf, C.BUTTON, r.center, max(2, cell // 2 - 1),
                                   0 if (x, y) in w.pressed else 2)
    ...
    foot = [f"cursor ({cursor[0]}, {cursor[1]})    blue = the plan    "
            "shaded = the drive    pale = the pad    ring = the button",
