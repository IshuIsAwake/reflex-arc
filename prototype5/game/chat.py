...
`goto` and `distance` are travel tools, `step` and `press` are the controller
buttons -- gemma asks, the world answers; `skills.py` owns them.
...
The system prompt promises exactly what is wired up, and a test holds it to the four
...
You have four skills and no others:

  goto(x, y, why)       drive there. One call drives the whole way.
  distance(x, y, why)   what that drive would cost. Spends nothing.
  step(direction, why)  drive exactly one cell north, south, east or west.
  press(why)            press the button on the cell you are standing on.
...
`step` is the controller buttons in your hands, but every step is a full round
trip over the link, so it is for the last cell and tight spots -- never for
journeys, which is what `goto` is for. `press` only does something standing on
a button cell, which is listed under WHAT YOU KNOW IS HERE; anywhere else it
does nothing and still costs the step.
