...
# The live route file: the current plan, and only ever the current one. Rewritten on
# every fresh `goto` and on every replan inside one, so a reader holds the rover's
# present intention rather than a queue of stale ones it has to catch up on. Emptied
# when there is no route, because the dangerous failure is a reader still executing
# the last good plan after the planner has given up on it.
#
# Relative paths resolve against the prototype directory, not the shell's -- `main.py`
# gets run from both. Only a live run writes it (`nav.publish` checks `world.recorder`),
...
#   teleport  step cell to cell (the default, and what every test assumes)
#   policy    drive each leg with the trained one-cell DQN
