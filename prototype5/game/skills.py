...
this is the file the rover implementation swaps while both sides stay put.
    ...
    step(direction, why)             drive exactly one cell. Costs a step
    press(why)                       press the button under the rover, if any

`goto` is travel; `step` is the controller buttons in the model's hands, one
round trip per cell -- slow, for the last cell and tight spots, never for
journeys. `press` only does something standing on a button cell.
...
# Compass only, never forward/left -- the rover has no facing in the model's
# world (chat.SYSTEM says so and a test holds it), so relative words would be
# unanswerable. Single letters accepted: the console and the model both shorten.
STEP_DIRS = {"north": (0, -1), "south": (0, 1), "east": (1, 0), "west": (-1, 0),
...
def _dir(v):
    """A compass direction, generously read and loudly refused.

    "North", "N", " n " all mean north. Anything else is a BAD_ARGS, because a
    step in an unknown direction is a step in a random one, and the model once
    spent a day walking somewhere it never meant.
    """
    if isinstance(v, bool) or v is None:
        raise BadArgs(f"direction must be north, south, east or west, got {v!r}")
    t = str(v).strip().lower()
    if t in STEP_DIRS:
        return t, STEP_DIRS[t]
    raise BadArgs(f"direction must be north, south, east or west, got {v!r}")
...
TOOLS = [
    ...
    {"type": "function", "function": {
        "name": "step",
        "description": (
            "Drive exactly one cell north, south, east or west from where the "
            "rover stands, lifting fog as it goes. Costs one step on a move, "
            "nothing on a refusal. One call is one round trip over the link, so "
            "this is for the last cell and tight spots -- never for journeys, "
            "which is what goto is for. Directions are compass words only: there "
            "is no facing, so forward and back mean nothing."),
        "parameters": {"type": "object", "properties": {
            "direction": {"type": "string", "description":
                          "north, south, east or west (n, s, e, w work too)"},
            "why": {"type": "string", "description":
                    "optional: one line on what you expect from this"},
        }, "required": ["direction"]}}},
    {"type": "function", "function": {
        "name": "press",
        "description": (
            "Press the button on the cell the rover is standing on. Costs one "
            "step. Anywhere else it does nothing and says so -- buttons are "
            "listed under WHAT YOU KNOW IS HERE, so check the view first."),
        "parameters": {"type": "object", "properties": {
            "why": {"type": "string", "description":
                    "optional: one line on what you expect from this"},
        }, "required": []}}},
...
# `goto`/`distance` carry coordinates, so the digit gate keeps prose off them ("goto is
# the right tool here" has no paren and no digit). `step`/`press` carry words or nothing,
# so any paren after them is a call -- prose never writes "step(".
CALL_SHAPED = re.compile(r"(\b(goto|distance)\s*\([^)]*\d|\b(step|press)\s*\()")
...
def written_call(text):
    ...
    `step`/`press` need no coordinates, so `step("north")` and `press()` are
    complete as written -- their first positional field is `direction`, not `x`.
    """
    for m in WRITTEN.finditer(text or ""):
        ...
        if name in ("step", "press"):
            for key, val in zip(("direction", "why"), positional):
...
def _stuck(c, history):
    ...
    **`distance` is exempt from the second rule, and dropping that exemption would
    ...
    if c.name in ("goto", "step") and not c.gained:
        drives = [c]
        for p in reversed(history):
            if p.name not in ("goto", "step"):
...
def call(world, name, args, history=()):
    ...
    if name == "goto":
        ...
        # One cell on the controller buttons. move() charges only on success,
        # so a refusal is detected exactly like nav's: steps uncharged.
        if world.day_over:
            r = nav.Result("OUT_OF_STEPS", at=world.pos, new=0)
            c.result = str(r)
        else:
            was_pos = world.pos
            world.move(dx, dy)
            if world.pos == was_pos:
                r = nav.Result("BUMPED", at=(was_pos[0] + dx, was_pos[1] + dy),
                               stopped=was_pos, new=0)
            else:
                c.gained = len(world.revealed)
                r = nav.Result("MOVED", steps=1, at=world.pos, new=c.gained)
            c.result = f"{r} -- {r.advice}" if r.advice else str(r)
    elif name == "press":
        # The button under the rover, if any. step_action charges the step and
        # records the press; anywhere else this is an honest nothing.
        if world.day_over:
            r = nav.Result("OUT_OF_STEPS", at=world.pos, new=0)
            c.result = str(r)
        else:
            kind = world.step_action(3)
            r = (nav.Result("PRESSED", steps=1, at=world.pos, new=0)
                 if kind == "pressed" else
                 nav.Result("NOTHING_TO_PRESS", steps=1, at=world.pos, new=0))
            c.result = f"{r} -- {r.advice}" if r.advice else str(r)
    else:
