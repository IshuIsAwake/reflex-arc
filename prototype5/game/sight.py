...
def buttons_known(w):
    """Buttons on ground already seen, so naming them gives away nothing.

    Buttons are not tiles -- they never appear on the grid above -- so they get
    their own line rather than a glyph. The first button sits inside the
    landing-site reveal, which is why there is usually one from the first view.
    Seen-ness goes through nav.known, the one gated door onto the grid.
    """
    return [f"button at {_c(b)}" + (", PRESSED" if b in w.pressed else "")
            for b in sorted(w.buttons) if nav.known(w.here, *b) is not None]
