...
def test_drives_that_buy_no_map_are_said_out_loud_even_when_they_differ():
    ...
    Rewritten 2026-09-04 when the rule became a rate over a window rather than a streak.
    Two separate things are now said and the test keeps them apart, because conflating
    them is how the old version passed while a quarter of a sol went on backtracking:
    ...
    # must not read as an unqualified success -- watched twice in one sol on 2026-09-04.
...
def test_the_schema_matches_what_is_wired_up():
    ...
    ok = check("exactly three skills", names == {"goto", "distance", "scout"}, str(names))
    ...
    # failure to arrive. Say it in the schema, before the first call, not only in the
    # answer afterwards.
