...
def test_the_prompt_promises_exactly_what_exists():
    ...
    ok &= check("names scout", "scout(" in chat.SYSTEM)
    # The two facts about the flyer that are not guessable from the verb: it spends the
    # same day the rover drives on, and it leaves the rover where it stands.
    ok &= check("says the flyer costs the day", "costs steps" in chat.SYSTEM)
    ok &= check("...and that it does not move the rover",
                "does not move the rover" in chat.SYSTEM)
